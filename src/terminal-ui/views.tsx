...
export function renderDiffTerminal(diff: Diff, columns = process.stdout.columns): string {
  const width = terminalWidth(columns)
  return renderToString(<DiffView diff={diff} width={width} />, { columns: width })
}
...
function DiffView({ diff, width }: { diff: Diff; width: number }) {
  ...
  return (
    <Box flexDirection="column">
      <Header
        ...
        meta={`${changed.length} changed  ·  ${regressions} regressions  ·  ${improvements} improvements`}
        width={width}
      />
      {diff.configChanges.length > 0 ? <ConfigChanges changes={diff.configChanges} /> : null}
      {changed.length === 0 ? (
...
function DiffDetails({ routes }: { routes: RouteDelta[] }) {
  const detailed = routes.filter((route) => route.cause || route.modules.length > 0 || route.newHoles.length > 0)
