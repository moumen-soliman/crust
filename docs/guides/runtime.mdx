...
Gate it exactly like [the widget](/guides/widget). When the panel is open it shows the live values
...
<CardGroup cols={2}>
  ...
  <Card title="Streaming waterfall" icon="water">
    When each Suspense boundary actually filled — [shell engine layer 3](/concepts/shell-engine).
  </Card>
</CardGroup>
...
On `pagehide` the collector beacons a compact sample. See [staging](/guides/staging) for the
endpoint.
