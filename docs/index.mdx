...
- route exports such as `dynamic`, `revalidate`, `runtime`, and `fetchCache`, plus `instant` and
  `prefetch` on Next 16.3
...
Every one of those questions is also answerable by the coding agent you already run — the same
stored evidence, served over MCP with no model or API key. See [Ask an agent](/docs/guides/agents).
