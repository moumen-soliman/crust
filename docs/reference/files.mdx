...
.perf/
  ...
  findings.jsonl           blocking findings + author verdicts
...
Each build snapshot also keeps cause chains, client-boundary and barrel costs, shared-cause groups,
build configuration, and the raw counts behind analysis coverage. These are stored with the build
because a future tool version must not reinterpret old evidence using newer analysis rules.
...
`crust ci` appends one row for every blocking breach. Each occurrence has a unique id to mark and a
content key for identical breaches. Mode and cache keys remain stable across runs; byte, growth, and
shell keys can change with the measured values included in their message:
...
The log is evidence for measuring disagreement, not a generated claim. Open findings do not enter
the rate, and an empty log remains unmeasured.
