...
```bash
npx @moumensoliman/crust diff [base] [head] [--build [cmd]]
```
...
```bash
npx @moumensoliman/crust diff develop chore/crust-ci --build
```

| Flag | Default | What it does |
| --- | --- | --- |
| `--build [cmd]` | off; `next build` when bare | Build both refs before comparing. Needs both positionals. |
| `--parallel` | `false` | Build the two refs at the same time; more CPU and disk at once. |
| `--cwd <dir>` | `pwd` | The app directory - the build target in each worktree, and the store anchor. |
| `--dist-dir <dir>` | `.next` | Build output, relative to the app directory inside each worktree. |
| `--keep-worktrees` | `false` | Leave the worktrees on disk to inspect a build. |

Quote a command with arguments: `--build 'pnpm --filter web build'`. In a monorepo, point `--cwd` at
...
```bash
npx @moumensoliman/crust diff develop feature --build 'pnpm install --frozen-lockfile && pnpm build'
```

A worktree contains committed files only. Untracked build inputs - `.env.local` above all - are not
...
No external requests: styles, script and data are inlined. The report opens with **Fix first** — the
same ranked findings `analyze` prints, worst first, each with its evidence and a concrete next step —
...
```bash
npx @moumensoliman/crust mcp [--cwd .]
```
...
[Ask an agent](/docs/guides/agents) for what each answers and the guarantees they hold to.

<Warning>
No tool builds, installs, or writes a snapshot, so an agent cannot publish a baseline or start a
production build. Answers only cover commits someone has already run `analyze` on.
</Warning>

The tool names, inputs and response shapes are not covered by the CLI stability rules yet — see
[COMPATIBILITY.md](https://github.com/moumensoliman/crust/blob/master/COMPATIBILITY.md#mcp-tool-surface).
...
```bash
npx @moumensoliman/crust ask                            # the tools and their arguments
npx @moumensoliman/crust ask build_findings
npx @moumensoliman/crust ask route_detail route=/dashboard
npx @moumensoliman/crust ask compare_builds base=<buildId> head=<buildId>
```

Arguments are `key=value` rather than flags, so a tool can gain an argument without colliding with
