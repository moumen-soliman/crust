...
`ci` writes one row per blocking breach. Each row has a fresh occurrence id (what you mark) and a
stable key (same failure across PRs). `rate` reports `disputed / (agreed + disputed)` and leaves the
rate unmeasured until something has been marked — an empty log is not a 0% disagreement rate.
