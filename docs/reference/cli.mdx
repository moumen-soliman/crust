...
```bash
npx @moumen/crust analyze [--json [file]] [--no-save]
```
...
```bash
npx @moumen/crust ci [ref] [--comment <file>]
```
...
```bash
npx @moumen/crust report [--out crust-report.html] [--open]
```
...
```bash
npx @moumen/crust history push [--remote origin]
npx @moumen/crust history fetch [--remote origin]
```
...
```bash
npx @moumen/crust prune [--dry-run]
```
...
```bash
npx @moumen/crust synthetic <baseUrl> [...routes] \
